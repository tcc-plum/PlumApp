...
def load_images_to_encodings(face_paths):
	...
	for idx,imagePath in enumerate(imagePaths):
		if imagePath.endswith('.jpg'):
			...
			face = img_to_encodings(cv2.imread(imagePath))
			if len(face) == 1:
				X.append(face[0])
...
def compare_faces(known_face_src,unknown_face_src,diff=0.5):
	known_face_encodings = img_to_encodings(cv2.imread(known_face_src))[0]
	unknown_face_encodings = img_to_encodings(cv2.imread(unknown_face_src))[0]
